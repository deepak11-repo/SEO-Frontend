...
function Heading5({ data }) {
    console.log('Inside Heading5');
    console.log(data);
            ...
            <>
                <div className="w-[99%] mb-5 mt-2">
                    <Alert severity="info" sx={{width: '100%'}}>
                        <p className="text-md tracking-wide leading-6 text-neutralDGrey font-medium">Currently, the website does not contain any H5 headings or associated data.</p>
                    </Alert>
                </div>
                <div className="w-full flex flex-col justify-center items-center">
    ...
    if (data.message === "optimized") {
        return (
            <>
                <div className="w-[99%] mb-5 mt-2">
                    <Alert severity="info" sx={{width: '100%'}}>
                        <p className="text-md tracking-wide leading-6 text-neutralDGrey font-medium">Current Header (H5) is already optimized with the specified keywords, therefore additional optimization is not required.</p>
                    </Alert>
                </div>
                <div className="flex items-center bg-white p-3 rounded-lg gap-3">
                    <div className="flex-1 text-justify tracking-wide leading-5">The H5 header is already fully optimized and meets all the required SEO criteria.</div>
            ...
            <div>
                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 shadow-lg">
                    <thead className="text-xs uppercase text-white bg-gray-900 tracking-wide">
                        <tr>
                            <th scope="col" className="px-6 py-3" style={{ width: '200px' }}>Heading (H5) <TooltipComponent msg={originalSubheadingsMsg}/></th>
                            <th scope="col" className="px-6 py-3" style={{ width: '230px' }}>Recommended Heading (H5) <span className="bg-[#82e87a] p-[3px] ml-1 rounded-sm text-[13px] text-gray-900 normal-case">Optimized</span> <TooltipComponent msg={recommendedSubheadingsMsg}/></th>
                    ...
                    <tbody>
                        {currentItems.map((item, index) => (
                            <tr key={index} className="bg-white border-b hover:bg-gray-50 text-neutralDGrey">
                                <td className="px-6 py-4 align-top text-left leading-6 tracking-wide font-medium">{item.h5}</td>
                                <td 
